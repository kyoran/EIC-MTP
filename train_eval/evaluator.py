    ...
    def evaluate(self, output_dir: str):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.dl):
                ...
                # Forward pass
                predictions, nei_num, max_dis, mean_dis = self.model(data['inputs'], is_val=True)
                # predictions = self.model(data['inputs'])    # pgp
                ...
                nei_num = nei_num.cpu().detach().squeeze().numpy()
                if nei_num == 0:
                    all_test_stat['agents_num'].append(1)
                    all_test_stat['max_dis'].append(0)
                    all_test_stat['mean_dis'].append(0)
                    # all_test_stat['density'].append(0)
                else:
                    all_test_stat['agents_num'].append(nei_num + 1)
                    all_test_stat['max_dis'].append(max_dis[0])
                    all_test_stat['mean_dis'].append(mean_dis[0])

                    # num / (d_max^2 * pi)
                    # all_test_stat['density'].append((nei_num + 1) / (one_max_dis * one_max_dis * np.pi))

                all_test_stat['min_ade_1'].append(agg_metrics['min_ade_1'])
                all_test_stat['min_ade_5'].append(agg_metrics['min_ade_5'])
                all_test_stat['min_ade_10'].append(agg_metrics['min_ade_10'])
                all_test_stat['min_fde_1'].append(agg_metrics['min_fde_1'])
                all_test_stat['min_fde_5'].append(agg_metrics['min_fde_5'])
                all_test_stat['min_fde_10'].append(agg_metrics['min_fde_10'])
                all_test_stat['miss_rate_1'].append(agg_metrics['miss_rate_1'])
                all_test_stat['miss_rate_5'].append(agg_metrics['miss_rate_5'])
                all_test_stat['miss_rate_10'].append(agg_metrics['miss_rate_10'])
                all_test_stat['pi_bc'].append(agg_metrics['pi_bc'])
                ##############

                # import pdb; pdb.set_trace()

        # 统计
        all_test_stat['dataset_index'] = dataset_index
        torch.save(all_test_stat, 'all_test_stat_v2.pt')
        # import pdb; pdb.set_trace()
