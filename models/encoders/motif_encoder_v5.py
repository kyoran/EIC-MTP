...
class Motif_Encoder_V5(PredictionEncoder):
    ...
    def forward(self, inputs, is_test=False, is_val=False):
        ...
        nbr_veh_enc, raw_nbr_veh_pos, nbr_veh_num = self.variable_size_tf_encode(nbr_vehicle_embedding, nbr_vehicle_masks, self.nbr_pos_enc, self.nbr_tf_enc, self.nbr_tf_norm, self.nbr_fc_enc)
        ...
        nbr_ped_enc, raw_nbr_ped_pos, nbr_ped_num = self.variable_size_tf_encode(nbr_ped_embedding, nbr_ped_masks, self.nbr_pos_enc, self.nbr_tf_enc, self.nbr_tf_norm, self.nbr_fc_enc)
        ...
        if is_val:
            explicit_rel_all_t, explicit_rel_agg, latent_rel, max_dis, mean_dis = self.extract_relationship(
        ...
        if is_test:
            ...
            return encodings, nbr_num, max_dis, mean_dis
    ...
    def extract_relationship(self,
            ...
            if tmp_nbr_num == 0:
                ...
                for ttt in range(traj_time):
                    ...
                    for iii in range(tmp_nbr_num):
                        one_dis = np.linalg.norm(
                            ego_pos_cpu[bbb, ttt] -
                            nbr_pos_cpu[iii]
                        )
                        if one_dis <= self.args['motif_influence_radius']:
                            A[iii, 0] = 1
                            A[0, iii] = 1
                        if one_dis > tmp_dis:
                            tmp_dis = one_dis
                        tmp_mean_dis.append(one_dis)
                    # nbr <-> nbr
                    for iii in range(tmp_nbr_num):
                    ...
                    # print("M:", M)
                    # motif diagonal degree matrix
                    d = M.sum(axis=1)
                    D = np.diag(d)
                    # print("d:", d)

                    if self.args['motif_type'] == 'motif':
                        tmp_m.append(M)
                ...
                max_dis.append(tmp_dis)
                mean_dis.append(np.mean(tmp_mean_dis))
        ...
        if is_val:
            return MMs, MMs_feas, LRs, max_dis, mean_dis
        else:
    ...
    @staticmethod
    def calc_motif(args, spp_enc, spp_agg, ego_pos, nbr_veh_pos, nbr_ped_pos, nbr_veh_num, nbr_ped_num, max_num):
        """
        target_agent_fea:
        nbr_veh_fea: [32, 84, 5, 6]
        nbr_ped_fea:
        nbr_veh_masks:
        nbr_ped_masks:
        """
        ...
        nbr_pos_cpu = nbr_pos.detach().cpu().numpy()
        # nbr_num
        nbr_num = nbr_veh_num + nbr_ped_num  # 会有某些batch里面没有邻居的情况

        # print("nbr_pos:", nbr_pos.shape)
        # print("nbr_num:", torch.sum(nbr_num).detach().cpu().item(), nbr_num)
        ...
        for bbb in range(batch_size):
            ...
            if tmp_nbr_num == 0:
                ...
                for ttt in range(traj_time):
                    tmp_m.append(
                        np.zeros(shape=(args['motif_default_size'],
                                        args['motif_default_size']), dtype=np.float32)
                    )
                ...
                for ttt in range(traj_time):
                    ...
                    if args['motif_type'] == 'motif':
                        tmp_m.append(M)

                    elif args['motif_type'] == 'motif Laplacian':
                        L = D - M
                        tmp_m.append(L)

                    elif args['motif_type'] == 'normalized motif Laplacian':
                        L = D - M
                        # print("L:", type(L))
                        # print("D:", type(D))
                        # print("M:", type(M))
            ...
            # print("tmp_m.shape:", tmp_m.shape)
            # tmp_m_fea = SPP(tmp_m, tmp_m.shape[0], tmp_m.shape[-2:], [16, 12, 8, 6, 4, 2])
            # tmp_m_fea = spp_enc(tmp_m)  # [1, 5, 24, 24] -> [1, 1, 24, 24]
            # tmp_m_fea = spp_agg(tmp_m_fea, tmp_m.shape[0], tmp_m.shape[-2:], [16, 12, 8, 6, 4, 2])
            tmp_m_fea = tmp_m_fea.sum(axis=1).unsqueeze(1)
            # print("tmp_m_fea.shape:", tmp_m_fea.shape)
            ...
            # MMs里面按batch存，一个batch里面是一组motif矩阵
            MMs.append(tmp_m)
            MMs_feas.append(tmp_m_fea)

            cur_nbr_start_idx += tmp_nbr_num

        # MMs_feas = torch.cat(MMs_feas, dim=0).to(device=device)
        # MMs_feas = torch.from_numpy(MMs_feas).to(torch.float32)
        return MMs, MMs_feas
    ...
    def variable_size_tf_encode(feat_embedding, masks, pos_enc, tf_enc, nbr_tf_norm, fc_enc):
        ...
        if len(seq_lens_batched) != 0:
            ...
            return encoding, raw_pos, nbr_num
