...
class Motif_Encoder_C1(PredictionEncoder):
    ...
    def forward(self, inputs, is_test=False):
        ...
        explicit_rel_all_t, explicit_rel_agg = self.extract_relationship(
    ...
    def agg_relationship(self,
        ...
        for bbb in range(len(fused_rel)):
            if nbr_veh_num[bbb]+nbr_ped_num[bbb] == 0:
                ...
                assert fused_rel[bbb].size(2) == nbr_veh_num[bbb]+nbr_ped_num[bbb]+1
    ...
    def fuse_relationship(self, explicit_rel, device):
        semantic_relationship = []

        for bbb in range(len(explicit_rel)):
